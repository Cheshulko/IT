...
public class UserFriendlyConsoleInterface {
	...
	case 1: {
		...
		System.out.println("Enter value for field " + name + " ("
			+ IntervalField.type + " St: "
			+ ifield.getStartIntvFrom() + " En: "
			+ ifield.getEndIntvTo() + "): ");
		String inputData = in.nextLine();
		tableInstanceBuilder.addIntervalFieldInstance(
			new IntervalFieldStringInstance(name, inputData));
	    }
	...
	case 1: {
		...
		if (BaseFieldType.contains(inputType)) {
		    ...
		    newTable = newTable.addTableIntervalField(new IntervalField(
			    st.charAt(0), en.charAt(0), inputName));
		} else {
